...
- Nested input is FIFO across all queues. Cancellation and queue changes immediately invalidate the old pass and clear stale queued input. Component additions wait for the next event, even in a later queue; removals stop responding immediately. Each duplicate component registration has independent interaction state. A component may remove itself from the render queue, but a callback must never unregister or replace its own event handler during invocation: `respond_mouse_event` runs the callable from the map that owns it, so mutating that entry destroys the executing object (undefined behavior). Register removals/replacements for a later event or tick instead.
